...
const CertCard = ({name,link,desc,img}) => {
  return (
    <div>
        <div onClick={()=> window.open(link,'_blank')} className="my-8 max-w-sm max-h-sm flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-max sm:max-w-max hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700 shadow-lg dark:bg-gray-800 hover:shadow-slate-500 transition duration-300 ease-in-out hover:scale-105">
        {img == "gmp" && 
        <Image className="object-cover rounded-t-lg md:h-auto md:w-auto sm:h-auto sm:w-auto md:rounded-none md:rounded-s-lg" width={200} height={200} quality={100} priority src="/assets/images/GMP.png" alt="image"/>
        }
        {img == "pde" && 
        <Image className="object-cover rounded-t-lg md:h-auto md:w-auto sm:h-auto sm:w-auto md:rounded-none md:rounded-s-lg" width={200} height={200} quality={100} priority src="/assets/images/PDE.png" alt="image"/>
        }
        {img == "gcp" && 
