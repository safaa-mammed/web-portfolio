...
const AboutCard = ({title, description, side, handleClick }) => {
  ...
  return (
    ...
    <div className="order-1 w-5/12"></div>
      <div className="z-20 flex items-center order-1 bg-white border border-color2 shadow-xl w-8 h-8 rounded-full">
        <h1 className="mx-auto font-semibold text-lg text-white"></h1>
      </div>
      <div className="order-1 bg-white border border-color2 rounded-lg shadow-2xl shadow-gray-200 w-5/12 px-6 py-4">
        <h3 className="mb-3 font-bold text-gray-900 text-xl ">{title}</h3>
        <p className="text-sm leading-snug text-justify tracking-wide text-gray-900 text-opacity-100">{description} </p>
        {handleClick && (
